...
class TestAbstractIndex(unittest.TestCase):
    def test_family_on_cls(self):
        self.assertIsInstance(index.AbstractIndex.family, index.FamilyProperty)
...
def test_suite():
    ...
    tests = unittest.TestSuite(
        (
            ...
            doctest.DocFileSuite(
                "setindex.rst", setUp=setUp64bit, tearDown=tearDown
            ),
            doctest.DocFileSuite(
                "valueindex.rst", setUp=setUp64bit, tearDown=tearDown
            ),
            doctest.DocFileSuite(
                "normalizedindex.rst", setUp=setUp64bit, tearDown=tearDown
            ),
            doctest.DocFileSuite(
                "globber.rst", setUp=setUp64bit, tearDown=tearDown
            ),
            doctest.DocFileSuite(
                "callablewrapper.rst", setUp=setUp64bit, tearDown=tearDown
            ),
            # legacy data support
            doctest.DocFileSuite(
                "legacy.rst", optionflags=doctest.ELLIPSIS, checker=checker
            ),
        )
    )
